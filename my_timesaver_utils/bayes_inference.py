...
def top_k_uncertainty(s, k=5, reverse=True):
    ...
    sorted_s = sorted(
        list(zip(torch.arange(len(s)), s)), key=lambda x: x[1], reverse=reverse
    )
...
@patch
def bayes_get_preds(
    self: Learner, ds_idx=1, dl=None, n_sample=10, act=None, with_loss=False, **kwargs
):
    ...
    with self.no_bar():
        for i in range(n_sample):
            pred, y = self.get_preds(
                ds_idx=ds_idx, dl=dl, act=act, with_loss=with_loss, cbs=cbs, **kwargs
            )
            # pred = n_dl x n_vocab
            preds.append(pred)
    ...
    with self.no_bar():
        for j in range(sample_size):
            inp, preds, _, dec_preds = self.get_preds(
                dl=dl, with_input=True, with_decoded=True, cbs=cbs
            )
...
# Cell
@patch
def bayes_predict_with_uncertainty(
    self: Learner,
    item,
    rm_type_tfms=None,
    with_input=False,
    ...
    res = self.bayes_predict(
        item,
        rm_type_tfms=rm_type_tfms,
...
# Cell
@patch
def bayes_build_inference_dfdlpreds(
    self: Learner, path, dataset, item_count=100, n_sample=10
):
