...
class EnhancedImageClassifierCleaner(GetAttr):
    def __init__(self, learn, file_mover=parent_move_file, **kwargs):
        ...
        self.widget = VBox(
            [
                HBox(
                    [
                        VBox([self.dd_cats, self.dd_ds]),
                        VBox([self.btn_apply_changes, self.btn_reset_changes]),
                    ]
                ),
                self.lbl_message,
                self.iw.widget,
    ...
    def recompute_loss(self, new_file):
        ...
        _, _, _, _, losses = self.learn.get_preds(
            dl=dl, with_input=True, with_loss=True, with_decoded=True
        )
        return losses[0]
