...
# Cell
def clear_prof_data(fname=None):
    "clear out profile data"
    global PROF_DATA
    if fname is None:
        PROF_DATA = {}
    else:
        PROF_DATA.pop(fname, None)
...
def start_record(name):
    ...
    if PROF_DATA[name][2] != 0:
        warnings.warn(
            f"function {name} start time not recorded because start time has already been recorded"
        )
        return
...
def end_record(name):
    ...
    if name not in PROF_DATA:
        warnings.warn(
            f"function {name} end time not recorded because start time not found"
        )
        return
...
def is_recording(name):
    ...
    return PROF_DATA[name][2] != 0 if name in PROF_DATA else False
